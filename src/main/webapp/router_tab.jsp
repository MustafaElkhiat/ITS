...
<div class="card card-outline-primary scroll">
    <div class="card-body">
        ...
        <c:if test="${fn:length(routerList) > 0}">
            <h5 class="card-title">Router <i class="fas fa-print"></i></h5>
            <jsp:include page="search_field.jsp">
                <jsp:param name="id" value="search_Router"/>
            </jsp:include>
            <div class="scroll">
                <table id="Router_table" class="table table-striped table-bordered" style="width: 2000px;">
                    <thead>
                    ...
                    <tbody id="Router_table_body">
                    <c:set var="count" value="1"/>
                    <c:forEach items="${routerList}" var="device">
                        ...
                        <tr class="${edit_device_privilege} ROU link" device-id="${device.id}">
                            <td>${count}</td>
